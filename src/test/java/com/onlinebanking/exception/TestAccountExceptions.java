...
public class TestAccountExceptions {
	...
	public void testCreateAccountInvalidDataException() throws Exception {
		...
		AccountDTO accountDto = new Gson().fromJson(responseBody, AccountDTO.class);
		yakshaAssert(currentTest(), (accountDto == null ? "true" : "false"), exceptionTestFile);
	}
	...
	public void testGetAccountByIdResourceNotFoundException() throws Exception {
		...
		when(this.accountService.getAccountById(accountDTO.getId()))
				.thenThrow(new ResourceNotFoundException("Account", "Account not found with id: ", accountDTO.getId()));
	...
	public void testDeleteAccountByIdResourceNotFoundException() throws Exception {
		...
		MvcResult result = mockMvc.perform(requestBuilder).andReturn();
		yakshaAssert(currentTest(), (result.getResponse().getContentAsString().contains(exResponse.getMessage()) ? "true" : "false"),
				exceptionTestFile);

	}

	@Test
	public void testSearchAccountByBogusUserName() throws Exception {
		AccountDTO accountDTO = MasterData.getAccountDTO();

		RequestBuilder requestBuilder = MockMvcRequestBuilders
				.get("/accounts/search?userName=" + accountDTO.getUser().getUsername())
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON);
